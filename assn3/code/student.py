...
def get_tiny_images(image_paths):
    ...
    for image_path in image_paths:
        im = resize(imread(image_path), (16, 16))
        im = im.reshape(-1, 1).squeeze()
        features.append(im)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    features = np.empty((0, cells_per_block[0] * cells_per_block[1] * 9))
    # features = []

    for image_path in image_paths:
        im = resize(imread(image_path), (64, 64))
        # im = resize(imread(image_path), (32, 32))
        hog_features = hog(im, cells_per_block = cells_per_block, 
            pixels_per_cell = pixels_per_cell)
        hog_features = hog_features.reshape(-1, cells_per_block[0] * cells_per_block[1] * 9)
        features = np.append(features, hog_features, axis = 0)
        
        # features = np.append(features, hog_features, axis = 0)
        
        # features.append(hog_features)

    # features = np.array(features).reshape(-1, cells_per_block[0] * cells_per_block[1] * 9)
    # features = np.array(features)

    print(hog_features.shape)
    print(features.shape)

    kmeans = KMeans(n_clusters = vocab_size, random_state = 0, max_iter = 100).fit(features)
...
def get_bags_of_words(image_paths):
    ...
    for image_path in image_paths:
        ...
        num_feature_vectors = hog_features.shape[0]
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    return lin_clf.predict(test_image_feats)
