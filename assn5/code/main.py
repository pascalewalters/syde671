...
def resize(old_image, factor):
    # size = (np.array(old_image.size) * factor).astype(int)
    # return skimage.transform.resize(old_image, size)
    return cv2.resize(old_image, None, fx = factor, fy = factor,
...
def main():
    ...
    pic_a = resize(pic_a, 0.25)
    pic_b = resize(pic_b, 0.37)
    ...
    # pic_a = resize(pic_a, 0.5)
    # pic_b = resize(pic_b, 0.5)
    ...
    # pic_a = resize(pic_a, 0.8)
    # pic_b = resize(pic_b, 1.0)
    ...
    # pic_a = resize(pic_a, 0.65)
    # pic_b = resize(pic_b, 0.65)
